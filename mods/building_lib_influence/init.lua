...
local function add_influence(pos, name, value)
    local groups = building_lib_influence.store:get(pos)
    if not groups then
        groups = {}
    end
    groups[name] = groups[name] or 0
    groups[name] = groups[name] + value
    building_lib_influence.store:set(pos, groups)
end
...
function building_lib_influence.get_groups(mapblock_pos)
    return building_lib_influence.store:get(mapblock_pos) or {}
end

function building_lib_influence.get(mapblock_pos, name)
    return building_lib_influence.get_groups(mapblock_pos)[name] or 0
end

function building_lib_influence.clear(mapblock_pos1, mapblock_pos2)
    mapblock_pos2 = mapblock_pos2 or mapblock_pos1
    apply_cube(mapblock_pos1, mapblock_pos2, function(mapblock_pos)
        building_lib_influence.store:set(mapblock_pos)
    end)
end
...
minetest.register_chatcommand("influence_info", {
    func = function(name)
        ...
        local data = building_lib_influence.store:get(mapblock_pos)
